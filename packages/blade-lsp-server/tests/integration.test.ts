...
import {
  ...
  getCompletions,
} from '../src/providers/completion.js';
...
describe('LSP Integration Tests - index.blade', () => {
  describe('Document Parsing', () => {
    ...
    it('should extract @props variables into scope', () => {
      ...
      const propNames = getVariablesAtOffset(doc.scope, 0).map(v => v.name);
    ...
    it('should mark @props variables as kind "prop"', () => {
      ...
      const titleVar = getVariablesAtOffset(doc.scope, 0).find(
        v => v.name === 'title'
      );
  ...
  describe('@props Completions Outside Loops', () => {
    it("should show the template definition's own props on line 5", () => {
      // Line 5 is inside <template:Card title! subtitle="Default">. A component
      // body renders with only its own props bound - `componentScope` builds a
      // fresh scope from the props and the globals - so offering the enclosing
      // file's `items` here offered a name that evaluates to nothing.
    ...
    it('should show every declared prop at the top level of the file', () => {
      // The <Card> usage on line 14 is in the document's own scope, where all
      // three declared props are bound.
      const cardLine = INDEX_BLADE_CONTENT.split('\n').findIndex(line =>
  ...
  describe('Scope Map Offsets', () => {
    it('records scope offsets in the coordinates of the file itself', () => {
      const doc = createDocument('test://index.blade', INDEX_BLADE_CONTENT);

      const offsets = doc.scope.segments.map(segment => segment.start);

      // @props is a node in the AST, so the first scope entry is the directive
      // itself at offset 0 - nothing is stripped and nothing is rebased.
      expect(offsets[0]).toBe(0);
      expect(offsets.length).toBeGreaterThan(1);

      // Sorted, and every offset indexes the real document.
      expect(offsets).toEqual([...offsets].sort((a, b) => a - b));
      for (const offset of offsets) {
        expect(offset).toBeGreaterThanOrEqual(0);
        expect(offset).toBeLessThanOrEqual(INDEX_BLADE_CONTENT.length + 1);
      }

      // The declared props are in scope inside the <Card> usage further down.
      const cardOffset = INDEX_BLADE_CONTENT.indexOf('<Card title=$title>');
      const names = getVariablesAtOffset(doc.scope, cardOffset).map(
        v => v.name
      );
