...
import { createDocument, parseDocument } from '../src/document.js';
...
describe('Diagnostic Provider', () => {
  ...
  describe('@props in the same coordinate system', () => {
    // @props used to be stripped from the source before parsing, and the line
    // of every diagnostic patched afterwards while the column and offset were
    // left alone - so every squiggle in a file with props landed on the wrong
    // character.
    const content = '@props(user)\n<div>\n  <span>${user.}</span>\n</div>';

    it('reports a parse error at its real line, column and offset', () => {
      const errors = getDocumentErrors(content);
      expect(errors.length).toBeGreaterThan(0);
    ...
    it('exposes the declared props on the document', () => {
      const doc = createDocument('test://test.blade', content);
      expect(doc.props.map(p => p.name)).toEqual(['user']);
    });

    it('reports no errors for a valid template that declares props', () => {
      expect(
        getDocumentErrors('@props(user)\n<div>${user.name}</div>')
      ).toHaveLength(0);
    });
  });
