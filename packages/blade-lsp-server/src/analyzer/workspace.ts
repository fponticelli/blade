...
export class WorkspaceManager {
  ...
  private readonly parseListeners: ((doc: BladeDocument) => void)[] = [];
   ...
   * Subscribe to parses.
   *
   * This is the one moment at which a document's AST, errors and scope become
   * visible, so it is the one place diagnostics may be published from. The
   * index used to be the only subscriber, and it published nothing - which is
   * why the last edit's errors never reached the editor.
   */
  onDocumentParsed(listener: (doc: BladeDocument) => void): void {
   ...
   */
  private handleDocumentParsed(doc: BladeDocument): void {
   ...
   *
   * The document manager is told too: it decides the parse debounce and
   * enforces `performance.maxFileSize`, and it used to keep whatever
   * configuration it was constructed with for the life of the session.
   */
  updateConfig(config: Partial<LspConfig>): void {
    ...
    this.documentManager.updateConfig(this.index.config);
  }

  /** Cancel every scheduled parse. */
  dispose(): void {
    this.documentManager.dispose();
    this.index.documents.clear();
    this.index.componentIndex.clear();
  }
