  ...
  "../../node_modules/vscode-languageserver-types/lib/umd/main.js"(exports2, module2) {
    (function(factory) {
      ...
      (function(TextDocumentItem2) {
        function create(uri, languageId, version, text2) {
...
function createWorkspaceIndex(config = DEFAULT_LSP_CONFIG) {
  return {
...
var expr = {
   ...
   */
  unary(operator, operand, location2) {
    return {
      kind: "unary",
      operator,
      operand,
   ...
   */
  binary(operator, left, right, location2) {
    return {
      kind: "binary",
      operator,
      left,
   ...
   */
  ternary(condition, truthy, falsy, location2) {
    return {
      kind: "ternary",
      condition,
      truthy,
   ...
   */
  member(object, pathSegments, hasWildcard = false, location2) {
    return {
      kind: "member",
      object,
      path: pathSegments,
...
var node = {
   ...
   */
  textLiteral(text2, location2) {
    return node.text([seg.literal(text2, location2)], location2);
  },
  /**
  ...
  component(opts) {
    return {
      kind: "component",
      name: opts.name,
      props: opts.props?.map((p) => ({
        name: p.name,
        value: p.value,
        location: p.location ?? loc()
      })) ?? [],
      children: opts.children ?? [],
      propPathMapping: opts.propPathMapping ?? /* @__PURE__ */ new Map(),
      location: opts.location ?? loc()
    };
  },
  /**
...
var match = {
   ...
   */
  expression(condition, body, location2) {
    return {
      kind: "expression",
      condition,
      body,
...
var text = {
  node: (opts) => node.text(opts.segments, opts.location),
...
var matchNode = {
  node: node.match,
  literalCase: (opts) => match.literal(opts.values, opts.body, opts.location),
  expressionCase: (opts) => match.expression(opts.condition, opts.body, opts.location)
};
var letNode = {
  node: node.letNode
};
var fragment = {
  node: (opts) => node.fragment(opts.children, opts.location)
};
var attribute = {
  static: (opts) => attr.static(opts.name, opts.value, opts.location),
  expr: (opts) => attr.expr(opts.name, opts.expr, opts.location),
  mixed: (opts) => attr.mixed(opts.name, opts.segments, opts.location),
...
function literal(value, type, location2) {
  return {
    kind: "literal",
    type,
    value,
...
function exprPath(segments, isGlobal, location2) {
  return {
    kind: "path",
    segments,
    isGlobal,
...
function pathIndex(index) {
  return { kind: "index", index };
}
function pathStar() {
  return { kind: "star" };
}
function unary(operator, operand, location2) {
  return {
    kind: "unary",
    operator,
    operand,
...
function binary(operator, left, right, location2) {
  return {
    kind: "binary",
    operator,
    left,
...
function ternary(condition, truthy, falsy, location2) {
  return {
    kind: "ternary",
    condition,
    truthy,
...
function call(callee, args, location2) {
  return {
    kind: "call",
    callee,
    args,
...
function functionExpr(params, body, location2) {
  return {
    kind: "function",
    params,
    body,
...
function arrayLiteral(elements, location2) {
  return {
    kind: "array",
    elements,
    location: location2 ?? loc()
...
function memberAccess(object, pathSegments, hasWildcard, location2) {
  return {
    kind: "member",
    object,
    path: pathSegments,
...
Context: ...${context}...`
  ...
  scanToken() {
    ...
    if (this.isWhitespace(char)) {
      if (char === "\n") {
        this.tokens.push(
          this.makeToken("NEWLINE" /* NEWLINE */, char, start, this.line - 1)
        );
      }
      return;
    }
    if (char === "$" && this.peek() === "{") {
      this.advance();
      this.tokens.push(this.makeToken("EXPR_START" /* EXPR_START */, "${", start));
      return;
    }
    if (char === "$") {
      this.tokens.push(this.makeToken("DOLLAR" /* DOLLAR */, char, start));
      return;
    }
    if (char === "@") {
      this.tokens.push(this.makeToken("AT" /* AT */, char, start));
      return;
    }
    ...
    switch (char) {
      case "(":
        this.tokens.push(this.makeToken("LPAREN" /* LPAREN */, char, start));
        break;
      case ")":
        this.tokens.push(this.makeToken("RPAREN" /* RPAREN */, char, start));
        break;
      case "{":
        this.tokens.push(this.makeToken("LBRACE" /* LBRACE */, char, start));
        break;
      case "}":
        this.tokens.push(this.makeToken("RBRACE" /* RBRACE */, char, start));
        break;
      case "[":
        this.tokens.push(this.makeToken("LBRACKET" /* LBRACKET */, char, start));
        break;
      case "]":
        this.tokens.push(this.makeToken("RBRACKET" /* RBRACKET */, char, start));
        break;
      case ".":
        this.tokens.push(this.makeToken("DOT" /* DOT */, char, start));
        break;
      case ",":
        this.tokens.push(this.makeToken("COMMA" /* COMMA */, char, start));
        break;
      case ":":
        this.tokens.push(this.makeToken("COLON" /* COLON */, char, start));
        break;
      case "#":
        this.tokens.push(this.makeToken("HASH" /* HASH */, char, start));
        break;
      case "_":
        this.tokens.push(this.makeToken("UNDERSCORE" /* UNDERSCORE */, char, start));
        break;
      case "+":
        this.tokens.push(this.makeToken("PLUS" /* PLUS */, char, start));
        break;
      case "-":
        this.tokens.push(this.makeToken("MINUS" /* MINUS */, char, start));
        break;
      case "*":
        this.tokens.push(this.makeToken("STAR" /* STAR */, char, start));
        break;
      case "/":
        if (this.peek() === ">") {
          this.advance();
          this.tokens.push(
            this.makeToken("TAG_SELF_CLOSE" /* TAG_SELF_CLOSE */, "/>", start)
          );
        } else {
          this.tokens.push(this.makeToken("SLASH" /* SLASH */, char, start));
        }
        break;
      case "%":
        this.tokens.push(this.makeToken("PERCENT" /* PERCENT */, char, start));
        break;
      case "!":
        if (this.peek() === "=") {
          this.advance();
          this.tokens.push(this.makeToken("BANG_EQ" /* BANG_EQ */, "!=", start));
        } else {
          this.tokens.push(this.makeToken("BANG" /* BANG */, char, start));
        }
        break;
      case "=":
        if (this.peek() === "=") {
          this.advance();
          this.tokens.push(this.makeToken("EQ_EQ" /* EQ_EQ */, "==", start));
        } else if (this.peek() === ">") {
          this.advance();
          this.tokens.push(this.makeToken("ARROW" /* ARROW */, "=>", start));
        } else {
          this.tokens.push(this.makeToken("EQUALS" /* EQUALS */, char, start));
        }
        break;
      case "<":
        if (this.peek() === "=") {
          this.advance();
          this.tokens.push(this.makeToken("LT_EQ" /* LT_EQ */, "<=", start));
        } else if (this.peek() === "/") {
          this.advance();
          this.tokens.push(this.makeToken("TAG_END_OPEN" /* TAG_END_OPEN */, "</", start));
        } else {
          this.tokens.push(this.makeToken("TAG_OPEN" /* TAG_OPEN */, char, start));
        }
        break;
      case ">":
        if (this.peek() === "=") {
          this.advance();
          this.tokens.push(this.makeToken("GT_EQ" /* GT_EQ */, ">=", start));
        } else {
          this.tokens.push(this.makeToken("TAG_CLOSE" /* TAG_CLOSE */, char, start));
        }
        break;
      case "&":
        if (this.peek() === "&") {
          this.advance();
          this.tokens.push(this.makeToken("AMP_AMP" /* AMP_AMP */, "&&", start));
        }
        break;
      case "|":
        if (this.peek() === "|") {
          this.advance();
          this.tokens.push(this.makeToken("PIPE_PIPE" /* PIPE_PIPE */, "||", start));
        }
        break;
      case "?":
        if (this.peek() === "?") {
          this.advance();
          this.tokens.push(
            this.makeToken("QUESTION_QUESTION" /* QUESTION_QUESTION */, "??", start)
          );
        } else {
          this.tokens.push(this.makeToken("QUESTION" /* QUESTION */, char, start));
        }
        break;
      default:
        break;
  ...
  scanNumber(start) {
    ...
    const value = this.source.substring(start, this.pos);
    this.tokens.push(this.makeToken("NUMBER" /* NUMBER */, value, start));
  }
  scanString(quote, start) {
    ...
    this.advance();
    const value = this.source.substring(start, this.pos);
    this.tokens.push(this.makeToken("STRING" /* STRING */, value, start));
  }
  ...
  makeToken(type, value, offset = this.pos - value.length, line = this.line, column = this.column - value.length) {
    return {
      type,
      value,
      line,
...
// ../blade/src/parser/expression-parser.ts
var ExpressionParser = class {
  ...
  parse() {
    try {
      const expr2 = this.parseExpression(0 /* NONE */);
      if (!this.isAtEnd() && this.peek().type !== "EOF" /* EOF */) {
      ...
      return { value: expr2, errors: this.errors };
    } catch (error) {
      return {
        value: null,
        errors: [
...
Current token: ${token.type} at line ${token.line}, column ${token.column}`
  ...
  parseNumber() {
    ...
    return literal(value, "number", this.getLocation(token));
  }
  parseString() {
    const token = this.advance();
    const value = token.value.slice(1, -1).replace(/\\(.)/g, "$1");
    return literal(value, "string", this.getLocation(token));
  }
  parseTrue() {
    const token = this.advance();
    return literal(true, "boolean", this.getLocation(token));
  }
  parseFalse() {
    const token = this.advance();
    return literal(false, "boolean", this.getLocation(token));
  }
  parseNull() {
    const token = this.advance();
    return literal(null, "nil", this.getLocation(token));
  }
  parseIdentifier() {
    ...
    const segments = [pathKey(name)];
    let hasWildcard = false;
    while (this.match("DOT" /* DOT */) || this.match("LBRACKET" /* LBRACKET */)) {
      if (this.previous().type === "DOT" /* DOT */) {
        segments.push(this.parsePathSegment());
      } else {
        const seg2 = this.parseIndexOrWildcard();
        if (seg2.kind === "star") hasWildcard = true;
        segments.push(seg2);
        this.consume("RBRACKET" /* RBRACKET */, "Expected ]");
      }
    }
    const pathNode = exprPath(segments, false, this.getLocation(start));
    if (hasWildcard) {
      return expr.wildcard(pathNode, this.getLocation(start));
    }
    return pathNode;
  }
  parsePath() {
    const start = this.peek();
    this.consume("DOLLAR" /* DOLLAR */, "Expected $");
    const isGlobal = this.match("DOT" /* DOT */);
    let hasWildcard = false;
    if (isGlobal && this.peek().type === "IDENTIFIER" /* IDENTIFIER */) {
      ...
      const pathNode = exprPath(segments, true, this.getLocation(start));
      if (hasWildcard) {
        return expr.wildcard(pathNode, this.getLocation(start));
      }
      return pathNode;
    }
    if (this.peek().type === "IDENTIFIER" /* IDENTIFIER */) {
      const segments = [this.parsePathSegment()];
      while (this.match("DOT" /* DOT */) || this.match("LBRACKET" /* LBRACKET */)) {
        if (this.previous().type === "DOT" /* DOT */) {
          segments.push(this.parsePathSegment());
  ...
  parsePathSegment() {
    const token = this.consume("IDENTIFIER" /* IDENTIFIER */, "Expected identifier");
    return pathKey(token.value);
  }
  ...
  isArrowFunction() {
    ...
    while (i < this.tokens.length) {
      ...
      if (!token || token.type === "EOF" /* EOF */) return false;
      if (token.type === "RPAREN" /* RPAREN */) break;
      i++;
    }
    if (i >= this.tokens.length) return false;
    i++;
    const nextToken = this.tokens[i];
    return nextToken?.type === "ARROW" /* ARROW */;
  }
  parseArrowFunction() {
    ...
    this.consume("RPAREN" /* RPAREN */, "Expected )");
    this.consume("ARROW" /* ARROW */, "Expected =>");
    const body = this.parseExpression(0 /* NONE */);
    return functionExpr(
      params,
      body,
  ...
  parseUnary() {
    ...
    const operand = this.parseExpression(9 /* UNARY */);
    return unary(
      operator.value,
      operand,
      this.getLocation(start)
  ...
  parseUnderscore() {
    const token = this.advance();
    return exprPath([pathKey("_")], false, this.getLocation(token));
  }
  parseArray() {
    const start = this.peek();
    ...
    if (!this.check("RBRACKET" /* RBRACKET */)) {
      do {
        elements.push(this.parseExpression(0 /* NONE */));
      } while (this.match("COMMA" /* COMMA */));
    }
    this.consume("RBRACKET" /* RBRACKET */, "Expected ]");
    return arrayLiteral(elements, this.getLocation(start));
  }
  ...
  parseTernary(left) {
    ...
    const truthy = this.parseExpression(0 /* NONE */);
    this.consume("COLON" /* COLON */, "Expected :");
    const falsy = this.parseExpression(0 /* NONE */);
    return ternary(left, truthy, falsy, this.getLocation(questionToken));
  }
  parseFunctionCall(name, start) {
    const args = [];
    if (!this.check("RPAREN" /* RPAREN */)) {
      do {
        args.push(this.parseExpression(0 /* NONE */));
      } while (this.match("COMMA" /* COMMA */));
    }
    this.consume("RPAREN" /* RPAREN */, "Expected )");
    return call(name, args, this.getLocation(start));
  }
  /**
  ...
  parseMemberAccess(left) {
    const segments = [];
    let hasWildcard = false;
    while (this.check("LBRACKET" /* LBRACKET */) || this.check("DOT" /* DOT */)) {
    ...
    return memberAccess(left, segments, hasWildcard, left.location);
  }
  // Parser rule tables
  getPrefixParser(type) {
    switch (type) {
      case "NUMBER" /* NUMBER */:
  ...
  skipNewlines() {
    while (!this.isAtEnd() && this.tokens[this.current]?.type === "NEWLINE" /* NEWLINE */) {
      this.current++;
    }
  }
  advance() {
  ...
  previous() {
    ...
    while (idx >= 0 && this.tokens[idx]?.type === "NEWLINE" /* NEWLINE */) {
      idx--;
    }
...
// ../blade/src/parser/template-parser.ts
...
Context: ...${context}...`
  ...
  parse() {
    ...
    while (!this.isAtEnd()) {
      ...
      const node2 = this.parseNode();
      if (node2) {
        if (node2.kind === "fragment" && node2.children.length > 0 && node2.children.every((c) => c.kind === "let")) {
          nodes.push(...node2.children);
        } else {
          nodes.push(node2);
        }
...
Context: ...${context}...`
  ...
  parseElement() {
    ...
    if (tagName.startsWith("template:")) {
      const componentName = tagName.substring(9);
      return this.parseComponentDefinition(componentName, startLoc);
    }
    const firstChar = tagName[0];
    if (tagName.length > 0 && firstChar && firstChar >= "A" && firstChar <= "Z") {
    ...
    while (!this.isAtEnd() && this.peek() !== ">" && this.peek() !== "/") {
      const prevPos = this.pos;
      const attr2 = this.parseAttribute();
      if (attr2) {
  ...
  parseComponent(tagName, startLoc) {
    const props = [];
    const propPathMapping = /* @__PURE__ */ new Map();
    this.skipWhitespace();
    while (!this.isAtEnd() && this.peek() !== ">" && this.peek() !== "/") {
      const prevPos = this.pos;
      ...
      if (prop) {
        ...
        if (prop.value.kind === "path" && !prop.value.isGlobal) {
          const pathSegments = prop.value.segments.filter(
            (seg2) => seg2.kind === "key"
          ).map((seg2) => seg2.key);
  ...
  parseComponentDefinition(name, startLoc) {
    const props = [];
    this.skipWhitespace();
    while (!this.isAtEnd() && this.peek() !== ">" && this.peek() !== "/") {
    ...
    if (this.peek() === "/" && this.peekNext() === ">") {
      ...
      this.componentDefinitions.set(name, componentDef2);
      return null;
    }
    this.consume(">");
    const body = [];
    ...
    while (!this.isAtEnd()) {
      if (this.peek() === "<" && this.peekNext() === "/") {
        ...
        while (tagEnd < this.source.length) {
          ...
          if (char === ">" || char === void 0 || this.isWhitespace(char)) {
            break;
          }
          tagEnd++;
        }
  ...
  parseComment() {
    ...
    const text2 = this.source.substring(contentStart, this.pos);
    if (this.peek() === "-") {
      this.advance();
      this.advance();
      this.advance();
  ...
  parseAttribute() {
    ...
    if (this.peek() === '"' || this.peek() === "'") {
      ...
      if (textBuffer || segments.length === 0) {
        segments.push(
          attribute.staticValue(textBuffer, this.getLocationFrom(startLoc))
        );
      }
      this.advance();
  ...
  parseText() {
    ...
    while (!this.isAtEnd()) {
      ...
      if (this.peek() === "@") {
        ...
        if (lookahead === "if" || lookahead === "for" || lookahead === "match" || lookahead === "let" || lookahead === "@" || // @@ code block
        lookahead === "endif" || lookahead === "endfor" || lookahead === "endmatch" || lookahead === "props" || lookahead === "component" || lookahead === "slot") {
          break;
        }
        textBuffer += this.advance();
        continue;
      ...
      if (this.peek() === "<") {
        const next = this.peekNext();
        if (this.isAlpha(next) || next === "/" || next === "!" || next === ">") {
          break;
        }
        textBuffer += this.advance();
        continue;
      ...
      if (this.peek() === "$" && this.peekNext() !== "{") {
        if (textBuffer) {
          segments.push(
            text.literalSegment(textBuffer, this.getLocationFrom(startLoc))
          );
          textBuffer = "";
        }
        const exprStart = this.pos;
        ...
        if (this.peek() === ".") {
          ...
          if (result.value) {
            segments.push(
              text.exprSegment(result.value, this.getLocationFrom(startLoc))
            );
          }
  ...
  parseMatchCase() {
    ...
    if (this.peek() === "*") {
      ...
      this.skipWhitespace();
      this.consume("{");
      const body2 = this.parseBlockBody();
      this.consume("}");
      return {
        isDefault: true,
        body: body2,
    ...
    if (this.peek() === "_") {
      ...
      return matchNode.expressionCase({
    ...
    const body = this.parseBlockBody();
    this.consume("}");
    return matchNode.literalCase({
      values,
      body,
      location: this.getLocationFrom(startLoc)
    });
  }
  parseFragment(startLoc) {
    this.consume(">");
    const children = [];
    while (!this.isAtEnd()) {
      if (this.peek() === "<" && this.peekNext() === "/") {
  ...
  parseCodeBlock(startLoc) {
    ...
    while (!this.isAtEnd() && this.peek() !== "}") {
      ...
      if (this.peekAhead(3) === "let") {
        ...
        const letNode2 = this.parseLet(this.getLocation(), {
          inCodeBlock: true
        });
        statements.push(letNode2);
      } else {
        while (!this.isAtEnd() && this.peek() !== ";" && this.peek() !== "\n") {
          this.advance();
        }
  ...
  parseIdentifier(allowDots = false) {
    const start = this.pos;
    while (this.isAlphaNumeric(this.peek()) || this.peek() === "-" || this.peek() === ":" || allowDots && this.peek() === ".") {
      this.advance();
    }
    return this.source.substring(start, this.pos);
  }
...
var PropsParser = class {
  ...
  parseDefaultValueExpression() {
    ...
    const expr2 = this.source.substring(start, this.pos).trim();
    return expr2 || null;
  }
  ...
  createLocation(startOffset, startLine, startColumn, endOffset) {
    return {
      start: {
        offset: startOffset,
        line: startLine,
  ...
  createResult(props, startOffset, startLine, startColumn) {
    this.skipWhitespace();
    const directive = {
      type: "PropsDirective",
      props,
...
// ../blade/src/lsp/analyzer/scope.ts
function analyzeScope(nodes, components) {
...
function analyzeComponentDefinition(name, def) {
  const props = def.props.map((p) => ({
    name: p.name,
    required: p.required,
    defaultValue: p.defaultValue !== void 0 ? String(p.defaultValue) : void 0
  }));
  const slots = [];
  function findSlots(nodes) {
    for (const node2 of nodes) {
      if (node2.kind === "slot") {
        slots.push({
          name: node2.name ?? null,
          location: node2.location
        });
      } else if (node2.kind === "element" || node2.kind === "fragment") {
        findSlots(node2.children);
      } else if (node2.kind === "if") {
        for (const branch of node2.branches) {
          findSlots(branch.body);
        }
        if (node2.elseBranch) {
          findSlots(node2.elseBranch);
        }
      } else if (node2.kind === "for") {
        findSlots(node2.body);
      } else if (node2.kind === "match") {
        for (const c of node2.cases) {
          findSlots(c.body);
        }
        if (node2.defaultCase) {
          findSlots(node2.defaultCase);
        }
      }
    }
  }
...
function analyzeNodes(nodes, context) {
  for (const node2 of nodes) {
    analyzeNode(node2, context);
  }
}
function analyzeNode(node2, context) {
  const { scope } = context;
  if (node2.kind !== "for" && node2.kind !== "if" && node2.kind !== "match") {
    recordVariablesAtLocation(node2.location, context);
  }
  switch (node2.kind) {
    case "text":
      break;
    case "element":
      if (isComponentName(node2.tag)) {
        const usage = {
          componentName: node2.tag,
          location: node2.location,
          props: {}
        };
        for (const attr2 of node2.attributes) {
          if (attr2.kind === "static" || attr2.kind === "expr") {
            usage.props[attr2.name] = attr2.location;
          }
        }
        scope.componentUsages.push(usage);
      }
      analyzeNodes(node2.children, context);
      break;
    case "if":
      analyzeIfNode(node2, context);
      break;
    case "for":
      analyzeForNode(node2, context);
      break;
    case "let":
      analyzeLetNode(node2, context);
      break;
    case "match":
      for (const c of node2.cases) {
      ...
      if (node2.defaultCase) {
        analyzeNodes(node2.defaultCase, context);
      }
      break;
    case "component":
      analyzeComponentNode(node2, context);
      break;
    case "fragment":
      analyzeNodes(node2.children, context);
      break;
...
function analyzeIfNode(node2, context) {
  context.nestingDepth++;
  for (const branch of node2.branches) {
    analyzeNodes(branch.body, context);
  }
  if (node2.elseBranch) {
    analyzeNodes(node2.elseBranch, context);
  }
  context.nestingDepth--;
}
function extractSourceVarName(itemsExpr) {
  if (itemsExpr.kind === "path") {
    ...
    if (segments.length > 0) {
      ...
      for (const seg2 of segments) {
        if (seg2.kind === "key") {
          pathParts.push(seg2.key);
        } else if (seg2.kind === "index") {
          break;
        } else {
          break;
        }
...
function analyzeForNode(node2, context) {
  context.nestingDepth++;
  const loopVariables = [...context.currentVariables];
  const sourceVarName = extractSourceVarName(node2.itemsExpr);
  loopVariables.push({
    name: node2.itemVar,
    kind: node2.iterationType === "of" ? "for-item" : "for-key",
    location: node2.location,
  ...
  context.scope.variables.set(node2.location.start.offset, [...loopVariables]);
  context.scope.variables.set(node2.location.end.offset - 1, [...loopVariables]);
  analyzeNodes(node2.body, bodyContext);
  context.nestingDepth--;
}
function analyzeLetNode(node2, context) {
  const variable = {
    name: node2.name,
    kind: node2.isGlobal ? "global" : "let",
    location: node2.location
  };
  context.currentVariables = [...context.currentVariables, variable];
  recordVariablesAtLocation(node2.location, context);
}
function analyzeComponentNode(node2, context) {
  const usage = {
    componentName: node2.name,
    location: node2.location,
    props: {}
  };
...
function recordVariablesAtLocation(location2, context) {
  const offset = location2.start.offset;
  const endOffset = location2.end.offset;
...
function getVariablesAtOffset(scope, offset) {
  let closest = -1;
  let closestVars = [];
...
// ../blade/src/lsp/document.ts
function createDocument(uri, content, version = 1) {
  const doc = {
    uri,
    version,
    content,
...
function parseDocument(doc) {
  ...
  let scope = result.errors.length === 0 && result.value.length > 0 ? analyzeScope(result.value, result.components) : createEmptyScope();
  const propsOffset = propsResult.remainingOffset;
  if (propsOffset > 0) {
    const adjustedVariables = /* @__PURE__ */ new Map();
...
function getOffset(content, line, character) {
...
function getWordAtOffset(content, offset) {
  let start = offset;
  let end = offset;
...
function getPathAtOffset(content, offset) {
  const isPathChar = (char) => /[\w$.[\]*]/.test(char);
  let start = offset;
  let end = offset;
  ...
  const path2 = content.slice(start, end);
  if (/^[.[\]*]+$/.test(path2)) {
    return null;
  }
  let basePath = path2.replace(/^\$/, "");
  basePath = basePath.replace(/\[\d+\]/g, "[]");
  basePath = basePath.replace(/\[\*\]/g, "[]");
  return { path: path2, start, end, basePath };
}
function isInsideExpression(content, offset) {
  let depth = 0;
...
function isAfterDirective(content, offset) {
  for (let i = offset - 1; i >= 0; i--) {
...
function isInsideTag(content, offset) {
  let inTag = false;
  let tagStart = -1;
  for (let i = offset - 1; i >= 0; i--) {
    const char = content[i];
...
var DocumentManager = class {
  ...
  config;
  onDocumentParsed;
  constructor(config, onDocumentParsed) {
    this.config = config;
...
// ../blade/src/lsp/analyzer/workspace.ts
var WorkspaceManager = class {
   ...
   */
  findComponentDefinition(componentName) {
    ...
    const component2 = doc.scope.components.find((c) => c.name === componentName);
    if (!component2) {
      return null;
    }
    return { uri, component: component2 };
  }
  /**
   ...
   */
  findComponentUsages(componentName) {
    const usages = [];
...
// ../blade/src/project/schema.ts
var import_promises = require("fs/promises");
var import_path = require("path");
async function loadProjectSchema(projectRoot) {
  const schemaPath = (0, import_path.join)(projectRoot, "schema.json");
  try {
    const content = await (0, import_promises.readFile)(schemaPath, "utf-8");
...
function extractSchemaProperties(schema, prefix = "") {
  const results = [];
  if (schema.properties) {
    for (const [name, prop] of Object.entries(schema.properties)) {
      ...
      if (prop.properties) {
        results.push(...extractSchemaProperties(prop, path2));
      }
      if (prop.items?.properties) {
        const itemProps = extractSchemaProperties(prop.items, `${path2}[]`);
...
function getSchemaCompletions(schema, path2) {
...
function getSchemaPropertyInfo(schema, path2) {
  const normalizedPath = path2.startsWith("$") ? path2.slice(1) : path2;
...
// ../blade/src/lsp/providers/completion.ts
function getCompletionContext(doc, offset) {
  ...
  if (isInsideExpression(content, offset)) {
    ...
    const isPathContext = pathInfo && (pathInfo.path.includes(".") || /\[\d*\]\.$/.test(pathInfo.path) || /\[\d*\]$/.test(pathInfo.path));
    if (isPathContext) {
...
function getCompletions(context, scope, projectContext) {
  const items = [];
...
function getExpressionCompletions(context, scope, projectContext) {
  const items = [];
...
function getPathCompletions(context, projectContext) {
  if (!projectContext?.schema) {
    return [];
  }
  const items = [];
  ...
  for (const prop of schemaCompletions) {
    ...
    items.push({
      label: propName,
      kind: prop.hasChildren ? 9 /* Module */ : 10 /* Property */,
...
function getSchemaBasedCompletions(context, projectContext) {
  if (!projectContext.schema) {
    return [];
  }
  const items = [];
...
function getDirectiveCompletions(_projectContext) {
  return [
    {
      label: "if",
      kind: 14 /* Keyword */,
...
function getDirectiveArgumentCompletions(context, projectContext) {
  const items = [];
  const content = context.document.content;
  const offset = getOffset(
    content,
...
function getHtmlTagCompletions(projectContext) {
  const items = [];
  if (projectContext) {
  ...
  items.push(
    ...commonTags.map((t, index) => ({
      ...
      kind: 10 /* Property */,
      detail: t.detail,
      sortText: (100 + index).toString().padStart(3, "0")
      // After components
    }))
  );
  return items;
}
function getHtmlAttributeCompletions(_context) {
  ...
  const items = globalAttributes.map((a, index) => ({
    label: a.attr,
    kind: 10 /* Property */,
    detail: a.detail,
...
function getComponentPropCompletions(context, projectContext) {
  const items = [];
  const content = context.document.content;
  const offset = getOffset(
    content,
...
function getBuiltinHelperCompletions() {
...
function getVariableKindLabel(kind) {
  switch (kind) {
    case "let":
...
function getPartialToken(content, offset) {
  let start = offset;
...
function isInsidePropsDirective(content, offset) {
  let depth = 0;
  for (let i = offset - 1; i >= 0; i--) {
    const char = content[i];
    if (char === ")") {
...
// ../blade/src/project/samples.ts
var import_promises2 = require("fs/promises");
var import_path2 = require("path");
var SAMPLES_DIR = "samples";
var JSON_EXTENSION = ".json";
async function loadProjectSamples(projectRoot) {
  const samplesPath = (0, import_path2.join)(projectRoot, SAMPLES_DIR);
  try {
    const entries = await (0, import_promises2.readdir)(samplesPath, { withFileTypes: true });
    const samples = [];
    for (const entry of entries) {
      if (!entry.isFile() || (0, import_path2.extname)(entry.name) !== JSON_EXTENSION) {
        continue;
      }
      const filePath = (0, import_path2.join)(samplesPath, entry.name);
      const name = (0, import_path2.basename)(entry.name, JSON_EXTENSION);
      try {
...
function extractSampleValues(samples) {
...
function extractValuesFromObject(obj, prefix, sampleName, values) {
  ...
  if (typeof obj !== "object") {
    ...
    const existing2 = values.get(prefix) || [];
    existing2.push(entry);
    values.set(prefix, existing2);
    return;
  }
  ...
  const objEntry = {
    sampleName,
    value: obj,
    displayValue: `Object(${Object.keys(obj).length} keys)`
  };
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    extractValuesFromObject(val, path2, sampleName, values);
  }
}
...
function getSampleValues(samples, path2) {
  const normalizedPath = path2.startsWith("$") ? path2.slice(1) : path2;
  return samples.values.get(normalizedPath) || [];
}
function formatSampleHint(values) {
  if (values.length === 0) {
    return "";
  }
  if (values.length === 1) {
...
// ../blade/src/lsp/providers/hover.ts
function getHoverInfo(doc, position, projectContext) {
  ...
  const propsContext = isInsidePropsDirective2(doc.content, offset);
  if (propsContext) {
...
function isSimpleVariableExpression(content, offset) {
  let i = offset - 1;
...
function isInsidePropsDirective2(content, offset) {
  const before = content.slice(Math.max(0, offset - 100), offset);
  const match2 = /@props\s*\([^)]*$/.test(before);
  return match2;
}
function isInsideDirectiveArgs(content, offset) {
  const before = content.slice(Math.max(0, offset - 200), offset);
  const unclosedMatch = before.match(/@(if|for|match)\s*\(([^)]*)$/);
  if (unclosedMatch) {
    const afterCursor2 = content.slice(offset);
    const closingParen = afterCursor2.indexOf(")");
    if (closingParen >= 0) {
      const fullContent = unclosedMatch[2] + afterCursor2.slice(0, closingParen);
...
function isInsideTemplateDefinition(content, offset) {
  const before = content.slice(Math.max(0, offset - 200), offset);
  const match2 = before.match(/<template:(\w+)[^>]*$/);
  if (match2) {
    return { componentName: match2[1] };
  }
  return null;
}
function getPropsVariableHover(propName, projectContext) {
...
function getDirectiveArgHover(_doc, _offset, name, context, projectContext) {
      ...
      if (name === itemVar) {
        if (projectContext?.schema && sourceVar) {
          ...
          if (sourceInfo?.type === "array") {
            ...
            const itemProps = projectContext.schema.properties.filter(
              (p) => p.path.startsWith(itemSchemaPath + ".")
            );
...
function getTemplatePropHover(propName, _context, projectContext) {
  const cleanName = propName.replace(/!$/, "");
...
function getSchemaVariableHover(name, projectContext) {
  if (projectContext?.schema) {
...
function getDirectiveHover(directive) {
  const directives = {
    if: {
      syntax: "@if(condition) { ... }",
      description: "Conditional rendering. Content is displayed only if the condition is truthy."
    },
    else: {
      syntax: "@else { ... }",
      description: "Else branch. Rendered when the preceding @if condition is falsy."
    },
    for: {
      syntax: "@for(item of items) { ... }\n@for(item, index of items) { ... }",
      description: "Loop iteration. Iterates over an array or object, rendering content for each item."
    },
    match: {
    ...
    "@": {
      ...
      description: "Variable declaration block. Declares local variables within the template."
    },
    component: {
      syntax: "@component Name(prop1, prop2)",
      description: "Component definition. Defines a reusable template component."
    },
    props: {
      syntax: "@props($prop1, $prop2, $prop3 = defaultValue)",
      description: "Declares the props that this component accepts. Props without default values are required."
    },
    slot: {
      syntax: "@slot name",
      description: "Declares a named slot for content projection. Used inside component definitions."
    }
  };
...
function getExpressionHover(doc, offset, name, projectContext) {
...
*Loop item variable (from @for)*`
      ...
      if (pathInfo && pathInfo.basePath.startsWith(varName + ".")) {
        const restPath = pathInfo.basePath.slice(varName.length + 1);
...
function getComponentHover(doc, componentName, projectContext) {
  const componentDef = doc.components.get(componentName);
  if (componentDef) {
    const props = componentDef.props.map((p) => {
      const required = p.required ? " (required)" : "";
      const defaultVal = p.defaultValue ? ` = ${typeof p.defaultValue === "string" ? `"${p.defaultValue}"` : "expr"}` : "";
...
`;
      if (projectComponent.props && projectComponent.props.length > 0) {
        const props = projectComponent.props.map((p) => `  ${p.name}${p.required ? " (required)" : ""}`).join("\n");
        contents += `\`\`\`blade
<${componentName}
${props}
/>
...
function formatVariableHover(variable) {
  const kindLabel = getVariableKindLabel2(variable.kind);
  const typeInfo = variable.valueType ? `: ${variable.valueType}` : "";
  return `**${variable.name}**${typeInfo}

*${kindLabel}*`;
}
function getVariableKindLabel2(kind) {
  switch (kind) {
    case "let":
...
function findVariableByNameAtOffset(scope, name, offset) {
...
// ../blade/src/project/discovery.ts
var import_promises3 = require("fs/promises");
var import_path3 = require("path");

// ../blade/src/project/utils.ts
function toPascalCase(str) {
  if (!str) return "";
  const words = str.replace(/([a-z])([A-Z])/g, "$1-$2").split(/[-_]+/).filter((word) => word.length > 0);
...
// ../blade/src/project/discovery.ts
var BLADE_EXTENSION = ".blade";
var ENTRY_FILE = "index.blade";
async function discoverComponents(projectRoot) {
  try {
    await (0, import_promises3.access)(projectRoot);
  } catch {
    throw new Error(`Project root does not exist: ${projectRoot}`);
  }
  const entryPath = (0, import_path3.join)(projectRoot, ENTRY_FILE);
  try {
    await (0, import_promises3.access)(entryPath);
  } catch {
    throw new Error(
...
async function scanDirectory(dir, namespace, components) {
  ...
  for (const entry of entries) {
    ...
    const fullPath = (0, import_path3.join)(dir, name);
    if (entry.isDirectory()) {
      ...
      const baseName = (0, import_path3.basename)(name, BLADE_EXTENSION);
      const componentName = toPascalCase(baseName);
      const tagName = namespace.length > 0 ? [...namespace, componentName].join(".") : componentName;
      const info = {
        tagName,
        filePath: fullPath,
        namespace: [...namespace],
...
// ../blade/src/lsp/project-context.ts
async function initializeProjectContext(projectRoot) {
  try {
...
var projectContexts = /* @__PURE__ */ new Map();
...
connection.onInitialized(() => {
  if (hasConfigurationCapability) {
    connection.client.register(
      import_node.DidChangeConfigurationNotification.type,
      void 0
    );
  }
  updateConfiguration();
  ...
  if (config) {
    const lspConfig = {
      diagnostics: {
        ...
        potentiallyUndefined: config.diagnostics?.potentiallyUndefined ?? "hint",
...
function getProjectRoot(uri) {
  const filePath = (0, import_url.fileURLToPath)(uri);
  return (0, import_path4.dirname)(filePath);
}
async function getProjectContext(uri) {
...
connection.onCompletion(
  async (params) => {
    ...
    connection.console.log(
      `[Completion] Returning ${completions.length} completions: ${completions.slice(0, 5).map((c) => c.label).join(", ")}...`
    );
